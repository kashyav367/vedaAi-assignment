...
function layoutAnswerBlocksOnPage(rawBlocks, pageNum) {
  ...
  const startY = pageNum === 1 ? 18.2 : 5.0;
  const endY = pageNum === 1 ? 81.5 : 95.0;
  const gap = 0.4;
  ...
  return answerBlocks.map((a) => {
    ...
    const computedHeight = Math.max(2.8, Number((availableHeight * proportion).toFixed(1)));

    const hasVisionY = typeof a.bbox?.y === 'number' && a.bbox.y > 0;
    const hasVisionH = typeof a.bbox?.height === 'number' && a.bbox.height > 0;

    const y = hasVisionY ? Number(a.bbox.y.toFixed(1)) : Number(cursorY.toFixed(1));
    const height = hasVisionH ? Number(a.bbox.height.toFixed(1)) : computedHeight;
...
console.log('--- ALL 10 QUESTIONS ACCURATE TOP AND BOTTOM COORDINATES ---');
laidOut.forEach((a) => {
