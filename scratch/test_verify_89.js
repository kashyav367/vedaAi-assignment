...
function layoutAnswerBlocksOnPage(rawBlocks, pageNum) {
  ...
  const endY = pageNum === 1 ? 89.0 : 95.0;
  const gap = 0.5;
  ...
  return answerBlocks.map((a, idx) => {
    ...
    const computedHeight = Number((availableHeight * (lines / totalLines)).toFixed(1));
...
console.log('--- PERFECT ACCURATE PHYSICAL COORDINATES WITH ENDY = 89.0 ---');
laidOut.forEach((a) => {
