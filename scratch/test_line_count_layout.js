...
function layoutAnswerBlocksOnPage(rawBlocks, pageNum) {
  ...
  const endY = pageNum === 1 ? 88.5 : 95.0;
  const gap = 0.5;

  const blockLines = answerBlocks.map((a) => {
    const len = (a.text || '').length;
    return Math.max(2, Math.ceil(len / 75));
  });
  ...
  return answerBlocks.map((a, idx) => {
    ...
    const height = Number((availableHeight * (lines / totalLines)).toFixed(1));
    const block = {
      ...a,
      page: pageNum,
      bbox: {
...
console.log('--- LINE COUNT ACCURATE COORDINATES FOR ALL Qs ---');
laidOut.forEach((a) => {
