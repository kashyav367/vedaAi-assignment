...
function layoutAnswerBlocksOnPage(rawBlocks, pageNum) {
  ...
  const endY = pageNum === 1 ? 89.5 : 95.0;
  const gap = 0.4;

  const blockLines = answerBlocks.map((a) => {
    const len = (a.text || '').length;
    return Math.max(3, Math.ceil(len / 65));
  });

  const totalLines = blockLines.reduce((sum, l) => sum + l, 0);
  const availableHeight = endY - startY - gap * (answerBlocks.length - 1);

  let cursorY = startY;
  return answerBlocks.map((a, idx) => {
    const lines = blockLines[idx];
    const computedHeight = Math.max(4.6, Number((availableHeight * (lines / totalLines)).toFixed(1)));
...
console.log('--- FIXED Q4 HEIGHT ENCLOSING BOTH LINES ---');
laidOut.forEach((a) => {
